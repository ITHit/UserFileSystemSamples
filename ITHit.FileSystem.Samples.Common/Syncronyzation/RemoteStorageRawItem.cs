    ...
    public class RemoteStorageRawItem
    {
        ...
        private async Task<Lock> LockAsync(FileMode lockFileOpenMode, LockMode lockMode = LockMode.Manual)
        {
            ...
            if (fileLock.IsNew())
            {
                ...
                IUserFileSystemItem userFileSystemItem = await virtualDrive.GetItemAsync<IUserFileSystemItem>(userFileSystemPath);
                ServerLockInfo lockInfo = await userFileSystemItem.LockAsync();

                // Save lock-token in lock-file.
                await fileLock.SetLockInfoAsync(lockInfo);

                // Set locked icon and all lock properties.
                await new UserFileSystemRawItem(userFileSystemPath).SetLockInfoAsync(lockInfo);
        ...
        private async Task UnlockAsync(Lock fileLock)
        {
            ...
            IUserFileSystemItem userFileSystemItem = await virtualDrive.GetItemAsync<IUserFileSystemItem>(userFileSystemPath);
            await userFileSystemItem.UnlockAsync(lockToken);

            // Remove lock icon.
            await new UserFileSystemRawItem(userFileSystemPath).SetLockInfoAsync(null);
