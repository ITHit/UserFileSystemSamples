                ...
                try
                {
                    ...
                    if (!FsPath.AvoidSync(userFileSystemPath))
                    {
                        ...
                        if (userFileSystemItem.IsMoved())
                        {
                            ...
                            await new RemoteStorageRawItem(userFileSystemOldPath, virtualDrive, this).MoveToAsync(userFileSystemPath);
                        }
                        else
                        {
                            ...
                            if (!userFileSystemItem.IsNew() && string.IsNullOrEmpty(userFileSystemOldPath))
                            {
                                ...
                                ServerLockInfo existingLock = isLocked ? await (await Lock.LockAsync(userFileSystemPath, FileMode.Open, LockMode.None, this)).GetLockInfoAsync() : null;
                                await new UserFileSystemRawItem(userFileSystemPath).SetLockInfoAsync(existingLock);
                            }
                        }
                    }
                }
                ...
                try
                {
                    if (!FsPath.AvoidSync(userFileSystemPath))
                    {
                        if (PlaceholderItem.GetItem(userFileSystemPath).IsNew())
                        {
                            ...
                            await RemoteStorageRawItem.CreateAsync(userFileSystemPath, virtualDrive, this);
                        }
                        else
                        {
